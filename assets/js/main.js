...
document.write(', if color text red/green, scss works')
...
//Моя версия, как по мне работает также как и препода(но не пропускает)
...
const regExp = new RegExp(/(\.jpg|\.jpeg|\.png)$/g, '');
...
// const regExp2 = new RegExp(/19.+|20.+|21.+/g, '');
// const regExp2 = new RegExp(/19.+|20.+|21.+|[0-9]{4,4}/g, '');
const regExp2 = new RegExp(/^(19\d\d|20\d\d|2100)$/g, '');
...
const regExp3 = new RegExp(/^[a-zA-Z]+\W?[a-zA-Z]+@[a-zA-Z\.]+\.[a-z]{2,3}$/g, '');
...
// const regExp4 = new RegExp(/[a-zA-z]{4,5}:\/\/[a-zA-Z-]+\W?[a-zA-Z]+\.[a-z]/g, '');

checkDomainUrl = (str) => {
    // return (/\s?(^[a-zA-z]{4,5}:\/\/[a-zA-Z-]+\W?[a-zA-Z]+\.[a-z]\s?)/g).test(str);
...
let regExp5 = new RegExp(/(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}/gi);

    str.match(regExp5).forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`))
